...
Official publishing is opt-in. When the user explicitly starts a YouTube upload, the chosen exported MP4, title, description, privacy setting, audience designation, and synthetic-media disclosure are sent to Google's official OAuth and YouTube Data API endpoints. OAuth client and token material is encrypted for the current Windows user with DPAPI or stored in the current Mac user's Keychain, and is never written to the repository. TikTok Direct Post is disabled until an audited supported integration exists.
